...
function UnsafeLazyImpl_create_Function0_k_(initializer as Object) as Object
    ...
    this.__get_initializer = UnsafeLazyImpl___get_initializer_k_
    this.__get__value = UnsafeLazyImpl___get__value_k_
    this.__set__value = UnsafeLazyImpl___set__value_AnyN_k_
    this.__get_value = UnsafeLazyImpl___get_value_k_
...
function UnsafeLazyImpl_isInitialized_k_() as Boolean
    return not __kotlin_identityEquals(m.__get__value(), UNINITIALIZED_VALUE_getInstance())
end function

function UnsafeLazyImpl_toString_k_() as String
    __when_tmp12 = invalid
    if m.isInitialized_k_() then
        __when_tmp12 = toString_AnyN_k_(m.__get_value())
    else if true then
...
function UnsafeLazyImpl___get_initializer_k_() as Object
    return m.initializer
end function

function UnsafeLazyImpl___get__value_k_() as Dynamic
    return m._value
end function

sub UnsafeLazyImpl___set__value_AnyN_k_(value as Dynamic)
    m._value = value
end sub

function UnsafeLazyImpl___get_value_k_() as Dynamic
    if __kotlin_identityEquals(m.__get__value(), UNINITIALIZED_VALUE_getInstance()) then
        m.__set__value(m.__get_initializer().invoke_k_())
    end if
...
function UNINITIALIZED_VALUE_getInstance() as Object
    if GetGlobalAA().UNINITIALIZED_VALUE_instance = invalid then
        GetGlobalAA().UNINITIALIZED_VALUE_instance = UNINITIALIZED_VALUE_create_k_()
    end if
    return GetGlobalAA().UNINITIALIZED_VALUE_instance
end function

function getValue_rLazy_AnyN_KProperty_k_(m as Object, thisRef as Dynamic, property as Object) as Dynamic
    return m.__get_value()
end function
...
function brsStructuralEquals_AnyN_AnyN_k_(a as Dynamic, b as Dynamic) as Boolean
    ...
    else if (Type(a) = "roSGNode") or (Type(b) = "roSGNode") then
        __when_tmp13 = false
    else if true then
        __when_tmp13 = (a = b)
    end if
