...
sub LinkedHashMap_putAll_Map_k_(from as Object)
    __iter_172 = from.get_entries().iterator_k_()
    while __iter_172.hasNext_k_()
        entry = __iter_172.next_k_()
...
function LinkedHashMap_equals_AnyN_k_(other as Dynamic) as Boolean
    ...
    __iter_173 = m.get_entries().iterator_k_()
    while __iter_173.hasNext_k_()
        entry = __iter_173.next_k_()
        otherMap = other
...
function LinkedHashMap_hashCode_k_() as Integer
    result = 0
    __iter_174 = m.get_entries().iterator_k_()
    while __iter_174.hasNext_k_()
        entry = __iter_174.next_k_()
...
function LinkedHashMap_LinkedEntrySet_removeAll_CollectionMutableEntry_k_(elements as Object) as Boolean
    modified = false
    __iter_180 = elements.iterator_k_()
