function Result_create_AnyN_k_(value as Dynamic) as Object
    ...
    this.__get_value = Result___get_value_k_
    this.__get_isSuccess = Result___get_isSuccess_k_
    this.__get_isFailure = Result___get_isFailure_k_
    this.value = value
    return this
end function
...
function Result_getOrNull_k_() as Dynamic
    __when_tmp0 = invalid
    if m.__get_isFailure() then
        __when_tmp0 = invalid
    else if true then
        __when_tmp0 = m.__get_value()
    end if
    return __when_tmp0
...
function Result_exceptionOrNull_k_() as Dynamic
    tmp0_subject = m.__get_value()
    __when_tmp1 = invalid
    if __kotlin_isInstanceOf(tmp0_subject, "Result_Failure") then
        __when_tmp1 = m.__get_value().__get_exception()
    else if true then
        __when_tmp1 = invalid
    end if
    return __when_tmp1
...
sub Result_throwOnFailure_k_()
    if __kotlin_isInstanceOf(m.__get_value(), "Result_Failure") then
        throw m.__get_value().__get_exception()
    end if
end sub
...
function Result_toString_k_() as String
    ...
    __when_tmp2 = invalid
    if __kotlin_isInstanceOf(tmp0_subject, "Result_Failure") then
        __when_tmp2 = (("Failure(" + m.__get_value().__get_exception().toString()) + ")")
    else if true then
        __when_tmp2 = (("Success(" + toString_AnyN_k_(m.__get_value())) + ")")
    end if
    return __when_tmp2
...
function Result___get_value_k_() as Dynamic
    return m.value
end function

function Result___get_isSuccess_k_() as Boolean
    return not __kotlin_isInstanceOf(m.__get_value(), "Result_Failure")
end function

function Result___get_isFailure_k_() as Boolean
    return __kotlin_isInstanceOf(m.__get_value(), "Result_Failure")
end function
...
function Result_Failure_create_Throwable_k_(exception as Object) as Object
    ...
    this.hashCode = Any_hashCode_k_
    this.toString = Any_toString_k_
    this.__get_exception = Result_Failure___get_exception_k_
    this.exception = exception
    return this
end function

function Result_Failure___get_exception_k_() as Object
    return m.exception
end function
