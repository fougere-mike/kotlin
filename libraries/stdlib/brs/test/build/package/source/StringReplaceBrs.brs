...
function replaceFirst_rStr_Str_Str_Z_Str_k_(m as String, oldValue as String, newValue as String, ignoreCase = false) as String
    ...
    return (substring_rStr_I_I_Str_k_(m, 0, index) + newValue) + substring_rStr_I_Str_k_(m, index + Len(oldValue))
end function
...
function replaceRange_rStr_I_I_CharSequence_Str_k_(m as String, startIndex as Integer, endIndex as Integer, replacement as Object) as String
    ...
    if endIndex > Len(m) then
        throw IndexOutOfBoundsException_create_StrN_IndexOutOfBoundsException_k_(((("End index (" + endIndex) + ") is greater than length (") + Len(m)) + ")")
    end if
    return (substring_rStr_I_I_Str_k_(m, 0, startIndex) + toString_AnyN_Str_k_(replacement)) + substring_rStr_I_Str_k_(m, endIndex)
end function
...
function replaceRange_rCharSequence_IntRange_CharSequence_Str_k_(m as Object, range as Object, replacement as Object) as String
    return replaceRange_rStr_IntRange_CharSequence_Str_k_(toString_AnyN_Str_k_(m), range, replacement)
end function
...
function replaceRange_rCharSequence_I_I_CharSequence_Str_k_(m as Object, startIndex as Integer, endIndex as Integer, replacement as Object) as String
    return replaceRange_rStr_I_I_CharSequence_Str_k_(toString_AnyN_Str_k_(m), startIndex, endIndex, replacement)
end function
