sub dateTimeTests_rTestRunner_k_(m as Object)
    m.suite_Str_Function1TestRunnerV_k_("DateTime", {invoke: function(m as Object) as Void
        ...
        m.test_Str_Function0V_k_("RoDateTime components", {invoke: function() as Void
            ...
            assertTrue_Z_StrN_k_(rangeTo_rI_I_IntRange_k_(1, 12).contains_I_Z_k_(month))
            assertTrue_Z_StrN_k_(rangeTo_rI_I_IntRange_k_(1, 31).contains_I_Z_k_(day))
        end function})
        m.test_Str_Function0V_k_("RoDateTime time components", {invoke: function() as Void
            ...
            assertTrue_Z_StrN_k_(rangeTo_rI_I_IntRange_k_(0, 23).contains_I_Z_k_(hours))
            assertTrue_Z_StrN_k_(rangeTo_rI_I_IntRange_k_(0, 59).contains_I_Z_k_(minutes))
            assertTrue_Z_StrN_k_(rangeTo_rI_I_IntRange_k_(0, 59).contains_I_Z_k_(seconds))
        end function})
        m.test_Str_Function0V_k_("RoDateTime milliseconds", {invoke: function() as Void
            dt = RoDateTime_create_RoDateTime_k_()
            dt.mark()
            ms = dt.getMilliseconds_I_k_()
            assertTrue_Z_StrN_k_(rangeTo_rI_I_IntRange_k_(0, 999).contains_I_Z_k_(ms))
        end function})
        m.test_Str_Function0V_k_("RoDateTime day of week", {invoke: function() as Void
            dt = RoDateTime_create_RoDateTime_k_()
            dt.mark()
            dow = dt.getDayOfWeek_I_k_()
            assertTrue_Z_StrN_k_(rangeTo_rI_I_IntRange_k_(0, 6).contains_I_Z_k_(dow))
        end function})
