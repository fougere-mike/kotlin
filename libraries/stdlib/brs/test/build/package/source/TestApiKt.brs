function __get__adapter_k_() as Dynamic
    return GetGlobalAA()._adapter
end function

sub __set__adapter_FrameworkAdapterN_k_(value as Dynamic)
    m._adapter = value
end sub

function adapter_k_() as Object
    if __get__adapter_k_() = invalid then
        __set__adapter_FrameworkAdapterN_k_(BareAdapter_create_k_())
    end if
    return __get__adapter_k_()
end function
