...
sub LayoutDirection_initEntries()
    if m.LayoutDirection_entriesInitialized = true then
        return
    end if
...
sub HorizAlignment_initEntries()
    if m.HorizAlignment_entriesInitialized = true then
        return
    end if
...
sub VertAlignment_initEntries()
    if m.VertAlignment_entriesInitialized = true then
        return
    end if
...
function ContainerNodeEntry___get_children_k_() as Object
end function
...
function LayoutGroupEntry_create_3vjkej_k_(id as String, layoutDirection = [LayoutDirection_initEntries(), m.LayoutDirection_horiz][1], horizAlignment = invalid, vertAlignment = invalid, itemSpacings = invalid, addItemSpacingAfterChild = invalid, translation = invalid, rotation = invalid, scale = invalid, scaleRotateCenter = invalid, opacity = invalid, visible = invalid, inheritParentOpacity = invalid, inheritParentTransform = invalid, clippingRect = invalid, renderGroup = invalid, focusable = invalid, renderPass = invalid, children = emptyList_k_()) as Object
    this = {}
...
function CustomComponentEntry_create_g4xuem_k_(id as String, componentType as String, customAttributes = emptyMap_k_(), translation = invalid, rotation = invalid, scale = invalid, scaleRotateCenter = invalid, opacity = invalid, visible = invalid, inheritParentOpacity = invalid, inheritParentTransform = invalid, clippingRect = invalid, renderGroup = invalid, focusable = invalid, renderPass = invalid, children = emptyList_k_()) as Object
    this = {}
    this.__type = "CustomComponentEntry"
    this.__proto = ["CustomComponentEntry", "ContainerNodeEntry", "NodeEntry", "CommonNodeDefaults", "CommonNodeProperties"]
    this.__id = __kotlin_nextObjectId()
    this.id = id
    ...
    this.renderPass = renderPass
    this.children = children
    this.equals = CustomComponentEntry_equals
    this.hashCode = CustomComponentEntry_hashCode
    this.toString = CustomComponentEntry_toString
    this.copy = CustomComponentEntry_copy
    this.component1 = CustomComponentEntry_component1
    this.component2 = CustomComponentEntry_component2
    this.component3 = CustomComponentEntry_component3
    this.component4 = CustomComponentEntry_component4
    this.component5 = CustomComponentEntry_component5
    this.component6 = CustomComponentEntry_component6
    this.component7 = CustomComponentEntry_component7
    this.component8 = CustomComponentEntry_component8
    this.component9 = CustomComponentEntry_component9
    this.component10 = CustomComponentEntry_component10
    this.component11 = CustomComponentEntry_component11
    this.component12 = CustomComponentEntry_component12
    this.component13 = CustomComponentEntry_component13
    this.component14 = CustomComponentEntry_component14
    this.component15 = CustomComponentEntry_component15
    this.component16 = CustomComponentEntry_component16
    return this
end function

function CustomComponentEntry_equals(other as Object) as Boolean
    if Type(other) <> "roAssociativeArray" then
        return false
    end if
    if other.__type <> "CustomComponentEntry" then
        return false
    end if
    if m.id <> other.id then
        return false
    end if
    if m.componentType <> other.componentType then
        return false
    end if
    if m.customAttributes <> other.customAttributes then
        return false
    end if
    if m.translation <> other.translation then
        return false
...
function CustomComponentEntry_hashCode() as Integer
    result = 1
    result = ((result * 31) + m.id)
    result = ((result * 31) + m.componentType)
    result = ((result * 31) + m.customAttributes)
    result = ((result * 31) + m.translation)
    result = ((result * 31) + m.rotation)
    result = ((result * 31) + m.scale)
    result = ((result * 31) + m.scaleRotateCenter)
...
function CustomComponentEntry_toString() as String
    return ((((((((((((((((((((((((((((((("CustomComponentEntry(id=" + m.id) + ", componentType=") + m.componentType) + ", customAttributes=") + m.customAttributes) + ", translation=") + m.translation) + ", rotation=") + m.rotation) + ", scale=") + m.scale) + ", scaleRotateCenter=") + m.scaleRotateCenter) + ", opacity=") + m.opacity) + ", visible=") + m.visible) + ", inheritParentOpacity=") + m.inheritParentOpacity) + ", inheritParentTransform=") + m.inheritParentTransform) + ", clippingRect=") + m.clippingRect) + ", renderGroup=") + m.renderGroup) + ", focusable=") + m.focusable) + ", renderPass=") + m.renderPass) + ", children=") + m.children) + ")"
end function

function CustomComponentEntry_copy(id = invalid, componentType = invalid, customAttributes = invalid, translation = invalid, rotation = invalid, scale = invalid, scaleRotateCenter = invalid, opacity = invalid, visible = invalid, inheritParentOpacity = invalid, inheritParentTransform = invalid, clippingRect = invalid, renderGroup = invalid, focusable = invalid, renderPass = invalid, children = invalid) as Object
    if id = invalid then
        id = m.id
    end if
    if componentType = invalid then
        componentType = m.componentType
    end if
    if customAttributes = invalid then
    ...
    return CustomComponentEntry_create_g4xuem_k_(id, componentType, customAttributes, translation, rotation, scale, scaleRotateCenter, opacity, visible, inheritParentOpacity, inheritParentTransform, clippingRect, renderGroup, focusable, renderPass, children)
end function

function CustomComponentEntry_component1() as String
    return m.id
end function

function CustomComponentEntry_component2() as String
    return m.componentType
end function

function CustomComponentEntry_component3() as Object
    return m.customAttributes
end function

function CustomComponentEntry_component4() as Dynamic
    return m.translation
end function

function CustomComponentEntry_component5() as Dynamic
    return m.rotation
end function

function CustomComponentEntry_component6() as Dynamic
    return m.scale
end function

function CustomComponentEntry_component7() as Dynamic
    return m.scaleRotateCenter
end function

function CustomComponentEntry_component8() as Dynamic
    return m.opacity
end function

function CustomComponentEntry_component9() as Dynamic
    return m.visible
end function

function CustomComponentEntry_component10() as Dynamic
    return m.inheritParentOpacity
end function

function CustomComponentEntry_component11() as Dynamic
    return m.inheritParentTransform
end function

function CustomComponentEntry_component12() as Dynamic
    return m.clippingRect
end function

function CustomComponentEntry_component13() as Dynamic
    return m.renderGroup
end function

function CustomComponentEntry_component14() as Dynamic
    return m.focusable
end function

function CustomComponentEntry_component15() as Dynamic
    return m.renderPass
end function

function CustomComponentEntry_component16() as Object
    return m.children
end function
