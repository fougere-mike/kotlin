sub stringBuilderTests_rTestRunner_k_(m as Object)
    m.suite_Str_Function1TestRunnerV_k_("StringBuilder", {invoke: function(m as Object) as Void
        ...
        m.test_Str_Function0V_k_("charAt", {invoke: function() as Void
            sb = StringBuilder_create_Str_k_("hello")
            assertEquals_AnyN_AnyN_StrN_k_("h", sb.get_I_k_(0))
            assertEquals_AnyN_AnyN_StrN_k_("e", sb.get_I_k_(1))
            assertEquals_AnyN_AnyN_StrN_k_("o", sb.get_I_k_(4))
        end function})
        m.test_Str_Function0V_k_("substring", {invoke: function() as Void
            sb = StringBuilder_create_Str_k_("hello world")
            assertEquals_AnyN_AnyN_StrN_k_("world", sb.substring_I_k_(6))
            assertEquals_AnyN_AnyN_StrN_k_("ello", sb.substring_I_I_k_(1, 5))
        end function})
        ...
        m.test_Str_Function0V_k_("isEmpty", {invoke: function() as Void
            sb = StringBuilder_create_k_()
            assertTrue_Z_StrN_k_(isEmpty_rCharSequence_k_(sb))
            sb.append_StrN_k_("x")
            assertFalse_Z_StrN_k_(isEmpty_rCharSequence_k_(sb))
        end function})
        m.test_Str_Function0V_k_("isNotEmpty", {invoke: function() as Void
            sb = StringBuilder_create_k_()
            assertFalse_Z_StrN_k_(isNotEmpty_rCharSequence_k_(sb))
            sb.append_StrN_k_("x")
            assertTrue_Z_StrN_k_(isNotEmpty_rCharSequence_k_(sb))
        end function})
        m.test_Str_Function0V_k_("appendLine", {invoke: function() as Void
            sb = StringBuilder_create_k_()
            sb.appendLine_StrN_k_("hello")
            sb.appendLine_StrN_k_("world")
            assertTrue_Z_StrN_k_(contains_rStr_Str_Z_k_(sb.toString(), "hello"))
            assertTrue_Z_StrN_k_(contains_rStr_Str_Z_k_(sb.toString(), "world"))
        end function})
    end function})
end sub
