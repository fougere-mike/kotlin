sub linkedHashSetTests_rTestRunner_k_(m as Object)
    m.suite_Str_Function1TestRunnerV_k_("LinkedHashSet", {invoke: function(m as Object) as Void
        m.test_Str_Function0V_k_("insertion order", {invoke: function() as Void
            set = LinkedHashSet_create_k_()
            set.add_AnyN_k_("three")
            set.add_AnyN_k_("one")
            set.add_AnyN_k_("two")
            list = toList_rIterable_k_(set)
            assertEquals_AnyN_AnyN_StrN_k_(listOf_Arr_k_(["three", "one", "two"]), list)
        end function})
        m.test_Str_Function0V_k_("no duplicates", {invoke: function() as Void
            set = LinkedHashSet_create_k_()
            assertTrue_Z_StrN_k_(set.add_AnyN_k_(1))
            assertTrue_Z_StrN_k_(set.add_AnyN_k_(2))
            assertFalse_Z_StrN_k_(set.add_AnyN_k_(1))
            assertEquals_AnyN_AnyN_StrN_k_(2, set.get_size())
            assertEquals_AnyN_AnyN_StrN_k_(listOf_Arr_k_([1, 2]), toList_rIterable_k_(set))
        end function})
        m.test_Str_Function0V_k_("constructor with collection", {invoke: function() as Void
            list = listOf_Arr_k_([3, 1, 2, 1, 3])
            set = LinkedHashSet_create_Collection_k_(list)
            assertEquals_AnyN_AnyN_StrN_k_(3, set.get_size())
            assertEquals_AnyN_AnyN_StrN_k_(listOf_Arr_k_([3, 1, 2]), toList_rIterable_k_(set))
        end function})
        m.test_Str_Function0V_k_("remove", {invoke: function() as Void
            set = LinkedHashSet_create_k_()
            set.add_AnyN_k_("one")
            set.add_AnyN_k_("two")
            set.add_AnyN_k_("three")
            assertTrue_Z_StrN_k_(set.remove_AnyN_k_("two"))
            assertFalse_Z_StrN_k_(set.remove_AnyN_k_("four"))
            assertEquals_AnyN_AnyN_StrN_k_(2, set.get_size())
            assertEquals_AnyN_AnyN_StrN_k_(listOf_Arr_k_(["one", "three"]), toList_rIterable_k_(set))
        end function})
        m.test_Str_Function0V_k_("contains", {invoke: function() as Void
            set = LinkedHashSet_create_k_()
            set.add_AnyN_k_(1)
            set.add_AnyN_k_(2)
            set.add_AnyN_k_(3)
            assertTrue_Z_StrN_k_(set.contains_AnyN_k_(2))
            assertFalse_Z_StrN_k_(set.contains_AnyN_k_(5))
        end function})
        m.test_Str_Function0V_k_("iterator", {invoke: function() as Void
            set = LinkedHashSet_create_k_()
            set.add_AnyN_k_(10)
            set.add_AnyN_k_(20)
            set.add_AnyN_k_(30)
            collected = mutableListOf_k_()
            __iter_2 = set.iterator_k_()
            while __iter_2.hasNext_k_()
                element = __iter_2.next_k_()
                collected.add_AnyN_k_(element)

            end while

            assertEquals_AnyN_AnyN_StrN_k_(listOf_Arr_k_([10, 20, 30]), collected)
        end function})
        m.test_Str_Function0V_k_("iterator remove", {invoke: function() as Void
            set = LinkedHashSet_create_k_()
            set.add_AnyN_k_("a")
            set.add_AnyN_k_("b")
            set.add_AnyN_k_("c")
            iter = set.iterator_k_()
            while iter.hasNext_k_()
                if iter.next_k_() = "b" then
                    iter.remove_k_()
                end if
            end while
            assertEquals_AnyN_AnyN_StrN_k_(2, set.get_size())
            assertEquals_AnyN_AnyN_StrN_k_(listOf_Arr_k_(["a", "c"]), toList_rIterable_k_(set))
        end function})
        m.test_Str_Function0V_k_("addAll", {invoke: function() as Void
            set = LinkedHashSet_create_k_()
            set.add_AnyN_k_(1)
            result = set.addAll_Collection_k_(listOf_Arr_k_([2, 3, 1]))
            assertTrue_Z_StrN_k_(result)
            assertEquals_AnyN_AnyN_StrN_k_(3, set.get_size())
            assertEquals_AnyN_AnyN_StrN_k_(listOf_Arr_k_([1, 2, 3]), toList_rIterable_k_(set))
        end function})
        m.test_Str_Function0V_k_("removeAll", {invoke: function() as Void
            set = LinkedHashSet_create_k_()
            set.addAll_Collection_k_(listOf_Arr_k_([1, 2, 3, 4, 5]))
            result = set.removeAll_Collection_k_(listOf_Arr_k_([2, 4]))
            assertTrue_Z_StrN_k_(result)
            assertEquals_AnyN_AnyN_StrN_k_(3, set.get_size())
            assertEquals_AnyN_AnyN_StrN_k_(listOf_Arr_k_([1, 3, 5]), toList_rIterable_k_(set))
        end function})
        m.test_Str_Function0V_k_("retainAll", {invoke: function() as Void
            set = LinkedHashSet_create_k_()
            set.addAll_Collection_k_(listOf_Arr_k_([1, 2, 3, 4, 5]))
            result = set.retainAll_Collection_k_(listOf_Arr_k_([2, 3, 6]))
            assertTrue_Z_StrN_k_(result)
            assertEquals_AnyN_AnyN_StrN_k_(2, set.get_size())
            assertEquals_AnyN_AnyN_StrN_k_(listOf_Arr_k_([2, 3]), toList_rIterable_k_(set))
        end function})
        m.test_Str_Function0V_k_("clear", {invoke: function() as Void
            set = LinkedHashSet_create_k_()
            set.addAll_Collection_k_(listOf_Arr_k_(["a", "b", "c"]))
            set.clear_k_()
            assertEquals_AnyN_AnyN_StrN_k_(0, set.get_size())
            assertTrue_Z_StrN_k_(set.isEmpty_k_())
        end function})
    end function})
end sub
