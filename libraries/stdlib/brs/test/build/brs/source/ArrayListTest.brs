sub arrayListTests_rTestRunner_k_(m as Object)
    m.suite_Str_Function1TestRunnerV_k_("ArrayList", {invoke: function(m as Object) as Void
        m.test_Str_Function0V_k_("create empty", {invoke: function() as Void
            ...
            assertTrue_Z_StrN_k_(list.isEmpty_k_())
            assertEquals_AnyN_AnyN_StrN_k_(0, list.get_size())
        end function})
        ...
        m.test_Str_Function0V_k_("create from collection", {invoke: function() as Void
            ...
            assertEquals_AnyN_AnyN_StrN_k_(1, list.get_I_k_(0))
            assertEquals_AnyN_AnyN_StrN_k_(2, list.get_I_k_(1))
            assertEquals_AnyN_AnyN_StrN_k_(3, list.get_I_k_(2))
        end function})
        m.test_Str_Function0V_k_("add elements", {invoke: function() as Void
            list = ArrayList_create_k_()
            assertTrue_Z_StrN_k_(list.add_AnyN_k_(1))
            assertTrue_Z_StrN_k_(list.add_AnyN_k_(2))
            assertTrue_Z_StrN_k_(list.add_AnyN_k_(3))
            assertEquals_AnyN_AnyN_StrN_k_(3, list.get_size())
            assertEquals_AnyN_AnyN_StrN_k_(1, list.get_I_k_(0))
            assertEquals_AnyN_AnyN_StrN_k_(2, list.get_I_k_(1))
            assertEquals_AnyN_AnyN_StrN_k_(3, list.get_I_k_(2))
        ...
        m.test_Str_Function0V_k_("add at index", {invoke: function() as Void
            ...
            assertEquals_AnyN_AnyN_StrN_k_(listOf_Arr_k_([1, 2, 3]), toList_rIterable_k_(list))
        end function})
        m.test_Str_Function0V_k_("get element", {invoke: function() as Void
            list = arrayListOf_Arr_k_([10, 20, 30])
            assertEquals_AnyN_AnyN_StrN_k_(10, list.get_I_k_(0))
            assertEquals_AnyN_AnyN_StrN_k_(20, list.get_I_k_(1))
            assertEquals_AnyN_AnyN_StrN_k_(30, list.get_I_k_(2))
        end function})
        ...
        m.test_Str_Function0V_k_("remove at index", {invoke: function() as Void
            ...
            assertEquals_AnyN_AnyN_StrN_k_(listOf_Arr_k_([1, 3]), toList_rIterable_k_(list))
        end function})
        m.test_Str_Function0V_k_("remove element", {invoke: function() as Void
            list = arrayListOf_Arr_k_([1, 2, 3])
            assertTrue_Z_StrN_k_(list.remove_AnyN_k_(2))
            assertFalse_Z_StrN_k_(list.remove_AnyN_k_(5))
            assertEquals_AnyN_AnyN_StrN_k_(listOf_Arr_k_([1, 3]), toList_rIterable_k_(list))
        end function})
        m.test_Str_Function0V_k_("contains", {invoke: function() as Void
            list = arrayListOf_Arr_k_([1, 2, 3])
            assertTrue_Z_StrN_k_(list.contains_AnyN_k_(2))
            assertFalse_Z_StrN_k_(list.contains_AnyN_k_(5))
        end function})
        ...
        m.test_Str_Function0V_k_("lastIndexOf", {invoke: function() as Void
            list = arrayListOf_Arr_k_([1, 2, 3, 2])
            assertEquals_AnyN_AnyN_StrN_k_(3, list.lastIndexOf_AnyN_k_(2))
            assertEquals_AnyN_AnyN_StrN_k_(-1, list.lastIndexOf_AnyN_k_(5))
        end function})
        ...
        m.test_Str_Function0V_k_("addAll", {invoke: function() as Void
            ...
            assertEquals_AnyN_AnyN_StrN_k_(listOf_Arr_k_([1, 2, 3, 4, 5]), toList_rIterable_k_(list))
        end function})
        ...
        m.test_Str_Function0V_k_("removeAll", {invoke: function() as Void
            ...
            assertEquals_AnyN_AnyN_StrN_k_(listOf_Arr_k_([1, 3, 5]), toList_rIterable_k_(list))
        end function})
        m.test_Str_Function0V_k_("retainAll", {invoke: function() as Void
            list = arrayListOf_Arr_k_([1, 2, 3, 4, 5])
            list.retainAll_Collection_k_(listOf_Arr_k_([2, 3, 6]))
            assertEquals_AnyN_AnyN_StrN_k_(listOf_Arr_k_([2, 3]), toList_rIterable_k_(list))
        end function})
        m.test_Str_Function0V_k_("subList", {invoke: function() as Void
            list = arrayListOf_Arr_k_([1, 2, 3, 4, 5])
            subview = list.subList_I_I_k_(1, 4)
            assertEquals_AnyN_AnyN_StrN_k_(listOf_Arr_k_([2, 3, 4]), toList_rIterable_k_(subview))
        end function})
