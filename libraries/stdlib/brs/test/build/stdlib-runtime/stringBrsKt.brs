...
function contentEquals_rCharSequenceN_CharSequenceN_Z_k_(m as Dynamic, other as Dynamic, ignoreCase as Boolean) as Boolean
    ...
    if not ignoreCase then
        ...
        progression_1 = until_rI_I_k_(0, __kotlin_charSequenceLength_CharSequenceN_k_(m))
        inductionVariable_1 = progression_1.__get_first()
        last_1 = progression_1.__get_last()
        if inductionVariable_1 <= last_1 then
                        i_1 = inductionVariable_1
            inductionVariable_1 = (inductionVariable_1 + 1)

            if not equals_rC_C_Z_k_(m.get_I_k_(i_1), other.get_I_k_(i_1), true) then
                return false
            end if
...
function indexOf_rStr_Str_I_Z_k_(m as String, string_ as String, startIndex = 0, ignoreCase = false) as Integer
    ...
    while true
        ...
        str = tmp2
        fromIndex = tmp4
        tmp_ret_0 = ((function(fromIndex, str, this)
            if Instr(fromIndex + 1, this, str) = 0 then return -1 else return Instr(fromIndex + 1, this, str) - 1
        end function)(fromIndex, str, this))
        exit while
    end while
    return tmp_ret_0
...
function lastIndexOf_rStr_Str_I_Z_k_(m as String, string_ as String, startIndex = __get_lastIndex_rStr_k_(m), ignoreCase = false) as Integer
    if startIndex = invalid then
        startIndex = __get_lastIndex_rStr_k_(m)
    end if
    if ignoreCase = invalid then
        ignoreCase = false
    end if
    ...
    tmp0 = m
    tmp2 = string_
    tmp4 = coerceAtMost_rI_I_k_(startIndex, Len(m))
    tmp_ret_0 = invalid
    ...
    while true
        ...
        searchIndex = 0
        while searchIndex <= fromIndex
            found = (function(searchIndex, str, this)
                if Instr(searchIndex + 1, this, str) = 0 then return -1 else return Instr(searchIndex + 1, this, str) - 1
            end function)(searchIndex, str, this)
            if (found < 0) or (found > fromIndex) then
                exit while
            end if
            lastIndex = found
...
function __get_lastIndex_rStr_k_(m as String) as Integer
    return Len(m) - 1
end function
...
function getOrElse_rStr_I_Function1IC_k_(m as String, index as Integer, defaultValue as Object) as Object
    ...
    if (index >= 0) and (index < Len(m)) then
        ...
        __when_tmp3 = defaultValue.invoke_AnyN_k_(index)
    end if
    return __when_tmp3
...
function concatToString_rCharArray_k_(m as Object) as String
    sb = StringBuilder_create_I_k_(m.__get_size())
    indexedObject = m
    inductionVariable = 0
...
function concatToString_rCharArray_I_I_k_(m as Object, startIndex = 0, endIndex = m.__get_size()) as String
    if startIndex = invalid then
        startIndex = 0
    end if
    if endIndex = invalid then
        endIndex = m.__get_size()
    end if
    if (startIndex < 0) or (endIndex > m.__get_size()) then
        throw IndexOutOfBoundsException_create_StrN_k_((((("startIndex: " + __kotlin_numToStr_I_k_(startIndex)) + ", endIndex: ") + __kotlin_numToStr_I_k_(endIndex)) + ", size: ") + __kotlin_numToStr_I_k_(m.__get_size()))
    end if
    if startIndex > endIndex then
        throw IllegalArgumentException_create_StrN_k_((("startIndex: " + __kotlin_numToStr_I_k_(startIndex)) + " > endIndex: ") + __kotlin_numToStr_I_k_(endIndex))
    end if
