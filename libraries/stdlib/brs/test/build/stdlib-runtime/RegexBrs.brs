...
function Regex_create_Str_SetRegexOption_Regex_k_(pattern as String, options as Object) as Object
    ...
    this.toString = Regex_toString_Str_k_
...
function Regex_matches_CharSequence_Z_k_(input as Object) as Boolean
    inputStr = toString_AnyN_Str_k_(input)
...
function Regex_containsMatchIn_CharSequence_Z_k_(input as Object) as Boolean
    return m.get_nativePattern().IsMatch_Str_Z_k_(toString_AnyN_Str_k_(input))
end function
...
function Regex_matchAt_CharSequence_I_MatchResultN_k_(input as Object, index as Integer) as Dynamic
    if (index < 0) or (index > Len(input)) then
        throw IndexOutOfBoundsException_create_StrN_IndexOutOfBoundsException_k_((("index out of bounds: " + index) + ", input length: ") + Len(input))
    end if
    inputStr = toString_AnyN_Str_k_(input)
...
function Regex_find_CharSequence_I_MatchResultN_k_(input as Object, startIndex = 0) as Dynamic
    if (startIndex < 0) or (startIndex > Len(input)) then
        throw IndexOutOfBoundsException_create_StrN_IndexOutOfBoundsException_k_((("Start index out of bounds: " + startIndex) + ", input length: ") + Len(input))
    end if
    inputStr = toString_AnyN_Str_k_(input)
...
function Regex_replace_CharSequence_Str_Str_k_(input as Object, replacement as String) as String
    if not contains_rStr_Str_Z_Z_k_(replacement, "\") and not contains_rStr_Str_Z_Z_k_(replacement, "$") then
        return m.get_nativePattern().ReplaceAll_Str_Str_Str_k_(toString_AnyN_Str_k_(input), replacement)
    end if
...
function Regex_replace_CharSequence_Function1MatchResultCharSequence_Str_k_(input as Object, transform as Object) as String
    ...
    if match = invalid then
        return toString_AnyN_Str_k_(input)
    end if
    lastStart = 0
    length = Len(input)
...
function Regex_replaceFirst_CharSequence_Str_Str_k_(input as Object, replacement as String) as String
    ...
    if not contains_rStr_Str_Z_Z_k_(replacement, "\") and not contains_rStr_Str_Z_Z_k_(replacement, "$") then
        return m.get_nativePattern().Replace_Str_Str_Str_k_(toString_AnyN_Str_k_(input), replacement)
    end if
    inputStr = toString_AnyN_Str_k_(input)
    ...
    sb.append_StrN_StringBuilder_k_(substring_rStr_I_I_Str_k_(inputStr, match.get_range().get_last() + 1, Len(inputStr)))
    return sb.toString()
end function
...
function Regex_split_CharSequence_I_ListStr_k_(input as Object, limit = 0) as Object
    ...
    for each match in matches
        result.add_AnyN_Z_k_(toString_AnyN_Str_k_(input.subSequence_I_I_CharSequence_k_(lastStart, match.get_range().get_start())))
        lastStart = (match.get_range().get_endInclusive() + 1)

    end for
    result.add_AnyN_Z_k_(toString_AnyN_Str_k_(input.subSequence_I_I_CharSequence_k_(lastStart, Len(input))))
    return result
end function
...
function Regex_splitToSequence_CharSequence_I_SequenceStr_k_(input as Object, limit = 0) as Object
    ...
    return Sequence_Function0IteratorAnyN_SequenceAnyN_k_({this: m, input: input, this: m, this: m, value: value, this: m, this: m, this: m, this: m, value: value, this: m, this: m, value: value, this: m, this: m, value: value, this: m, limit: limit, this: m, invoke: function() as Object
        return Anon_6e2becc2_create_Anon_k_()
    end function})
end function
...
function Regex_createMatchResult_Str_Arr_I_MatchResult_k_(input as String, matchArray as Object, matchIndex as Integer) as Object
    return Anon_3a8c4bab_create_Anon_k_()
end function
...
function substituteGroupRefs_MatchResult_Str_Str_k_(match as Object, replacement as String) as String
    ...
    while index < Len(replacement)
        __incr_tmp_8 = index
        index = (__incr_tmp_8 + 1)

        char = replacement.get_I_C_k_(__incr_tmp_8)
        if char = "\" then
            ...
            __incr_tmp_9 = index
            index = (__incr_tmp_9 + 1)

            result.append_C_StringBuilder_k_(replacement.get_I_C_k_(__incr_tmp_9))
        else if char = "$" then
            if index = Len(replacement) then
            ...
            if replacement.get_I_C_k_(index) = "{" then
                endIndex = readGroupName_rStr_I_I_k_(replacement, index)
                ...
                if (endIndex = Len(replacement)) or (replacement.get_I_C_k_(endIndex) <> "}") then
                ...
                if not rangeTo_rC_C_CharRange_k_("0", "9").contains_C_Z_k_(replacement.get_I_C_k_(index)) then
...
function readGroupIndex_rStr_I_I_I_k_(m as String, startIndex as Integer, groupCount as Integer) as Integer
    ...
    while (index < Len(m)) and rangeTo_rC_C_CharRange_k_("0", "9").contains_C_Z_k_(m.get_I_C_k_(index))
