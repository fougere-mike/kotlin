...
kotlin {
    ...
    sourceSets {
        ...
        // BRS source set is configured in brs-target.gradle (only when KGP has BRS support)
    }

    // BRS target - only available when KGP has BRS support
    // IMPORTANT: This must be AFTER sourceSets {} so assertionsCommonMain/annotationsCommonMain exist
    // See CLAUDE.md "Bootstrap Architecture" section for details
    //
    // The BRS configuration is in a separate Groovy script (brs-target.gradle) because:
    // - The `brs {}` DSL function is generated by KGP and only exists when KGP has BRS support
    // - Kotlin DSL scripts are compiled before execution - if `brs` doesn't exist, compilation fails
    // - Groovy uses dynamic dispatch - method resolution happens at runtime, not compile time
    if (kgpHasBrsSupport) {
        apply(from = "brs-target.gradle")
    } else {
