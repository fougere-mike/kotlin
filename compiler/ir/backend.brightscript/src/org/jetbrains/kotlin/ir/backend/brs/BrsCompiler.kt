...
class BrsCompiler(
    ...
    fun compile(irModule: IrModuleFragment): BrsModuleCompilationResult {
        ...
        // SharedService static dispatch: build the whole-module registry
        // (shared classes → concrete descendants) AFTER the manifest pass —
        // class-name uniquification is first-come, and the manifest pass must
        // stay the first exhaustive getBrsName walk over the module.
        context.sharedDispatchRegistry = BrsSharedDispatchRegistry.build(loweredModule, context)
            ...
            when (member) {
                ...
                is IrProperty -> {
                    ...
                    member.getter?.let { getter ->
                        val getterName = context.getBrsName(getter)
                        manifest[getterName] = outputFileName
                        if (sharedDispatcherCandidate(getter, context)) {
                            manifest[getterName + SHARED_DISPATCH_SUFFIX] = outputFileName
                        }
                    }
                    member.setter?.let { setter ->
                        val setterName = context.getBrsName(setter)
                        manifest[setterName] = outputFileName
                        if (sharedDispatcherCandidate(setter, context)) {
                            manifest[setterName + SHARED_DISPATCH_SUFFIX] = outputFileName
                        }
                    }
                }
                is IrClass -> {
