...
/**
 * Transforms returnable blocks to use temp variables for return values.
 *
 * This is needed because BrightScript doesn't support labeled returns or
 * returning values from within nested blocks. After inline function expansion,
 * the inliner creates IrReturnableBlock with IrReturn statements targeting
 * the block. These need to be converted to assignments + return Unit.
 *
 * ```
 * returnable_block {
 *   ...
 ...
 * ```
 *
 * Based on JsReturnableBlockLowering from the JS backend.
 */
class BrsReturnableBlockLowering(val context: CommonBackendContext) : BodyLoweringPass {
    override fun lower(irBody: IrBody, container: IrDeclaration) {
        container.transform(BrsReturnableBlockTransformer(context), null)
    }
}

class BrsReturnableBlockTransformer(val context: CommonBackendContext) : IrElementTransformerVoidWithContext() {
