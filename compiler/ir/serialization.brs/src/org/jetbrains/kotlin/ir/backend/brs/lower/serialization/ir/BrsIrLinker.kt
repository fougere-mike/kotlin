...
/**
 * IR linker for BrightScript target.
 *
 * Links IR modules from klib dependencies for BrightScript compilation.
 */
class BrsIrLinker(
    ...
    override fun createModuleDeserializer(
        ...
        return BrsModuleDeserializer(moduleDescriptor, klib, strategyResolver, libraryAbiVersion, klib.libContainsErrorCode)
    }

    private val deserializedFilesInKlibOrder = mutableMapOf<IrModuleFragment, List<IrFile>>()

    private inner class BrsModuleDeserializer(
        moduleDescriptor: ModuleDescriptor,
        klib: IrLibrary,
        strategyResolver: (String) -> DeserializationStrategy,
    ...
    fun moduleDeserializer(moduleDescriptor: ModuleDescriptor): IrModuleDeserializer {
        return deserializersForModules[moduleDescriptor.name.asString()]
            ?: error("Deserializer for $moduleDescriptor not found")
    }
