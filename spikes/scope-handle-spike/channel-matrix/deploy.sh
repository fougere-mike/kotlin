#!/usr/bin/env bash
# Deploy the scope-handle channel-matrix spike (raw BRS) and capture [SPIKE] verdicts.
# Harness: raw-BRS zip + curl sideload mirrored from spikes/task-node-spike/deploy.sh,
# plus the console preflight + device-OS query from spikes/render-thread-queue-spike/deploy.sh.
