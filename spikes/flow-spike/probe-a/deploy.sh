#!/usr/bin/env bash
# Deploy the flow-spike Probe A (global-node doorbell, raw BRS) and capture
# [SPIKE] verdicts. Harness mirrored from spikes/scope-handle-spike/
...
( sleep "${CAPTURE_SECONDS:-30}" | telnet "$IP" 8085 ) > spike-output.txt 2>/dev/null &
CAPTURE_PID=$!
sleep 1
...
# run. The first run lives at ../capture-probe-a.txt; every subsequent run gets
# the next free -runN suffix.
if [[ ! -e "../capture-probe-a.txt" ]]; then
    CAPTURE_OUT="../capture-probe-a.txt"
else
    N=2
    while [[ -e "../capture-probe-a-run${N}.txt" ]]; do N=$((N+1)); done
    CAPTURE_OUT="../capture-probe-a-run${N}.txt"
fi
